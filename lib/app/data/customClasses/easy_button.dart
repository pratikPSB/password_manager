...
class _EasyButtonState extends State<EasyButton> with TickerProviderStateMixin {
  ...
  Widget _buildChild(BuildContext context) {
    ...
    var buttonColor =
        widget.buttonColor != Colors.blue ? widget.buttonColor : Get.theme.colorScheme.primary;
  ...
  Widget _buildChildren(BuildContext context) {
    double contentGap = widget.type == EasyButtonType.text ? 0.0 : widget.contentGap;
    ...
    if (widget.useWidthAnimation) {
      ...
      _forward((status) {
        if (status == AnimationStatus.dismissed) {
          _toDefault();
          if (onIdle != null && (onIdle is VoidCallback || onIdle is FormFieldValidator)) {
            onIdle();
          }
        }
      });
